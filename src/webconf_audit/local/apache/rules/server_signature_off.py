...
RULE_ID = "apache.server_signature_not_off"


@rule(
    rule_id=RULE_ID,
    title="ServerSignature not set to Off",
    severity="low",
    description="Apache config does not define an effective 'ServerSignature Off' directive.",
    recommendation="Add 'ServerSignature Off' in the effective Apache scope.",
    category="local",
    server_type="apache",
    tags=("disclosure",),
    order=323,
)
def find_server_signature_off(config_ast: ApacheConfigAst) -> list[Finding]:
    ...
    directive = build_server_effective_config(config_ast).directives.get("serversignature")
    if directive is None:
        return [
            Finding(
                ...
                severity="low",
                description=(
                    "Apache config does not define an effective 'ServerSignature Off' "
                    "directive."
                ),
                recommendation="Add 'ServerSignature Off' in the Apache config.",
    ...
    return [
        Finding(
            ...
            severity="low",
            description=(
                "Apache config sets effective 'ServerSignature' to "
                f"'{configured_value}' instead of 'Off'."
            ),
            recommendation="Set the effective directive to 'ServerSignature Off'.",
...
def _evaluate_virtualhost(
    ...
    ).directives.get("serversignature")
    ...
    if directive is not None:
        ...
        description = (
            ...
            f"'ServerSignature' to '{configured_value}' instead of 'Off'."
        )
        location = SourceLocation(
            mode="local",
            kind="file",
    ...
    return [
        Finding(
            ...
            recommendation="Set the effective VirtualHost directive to 'ServerSignature Off'.",
            location=location,
        )
    ]
...
__all__ = ["find_server_signature_off"]
