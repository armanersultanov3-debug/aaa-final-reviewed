...
DENY_WRAPPER_BLOCKS = REQUIRE_METHOD_WRAPPER_BLOCKS | METHOD_RESTRICTION_BLOCKS
...
def _location_has_method_restriction(location: ApacheBlockNode) -> bool:
    for child in location.children:
        ...
        if child_name in METHOD_RESTRICTION_BLOCKS and _block_denies_access(child):
            return True
        if child_name in REQUIRE_METHOD_WRAPPER_BLOCKS and (
            _location_has_method_restriction(child)
        ):
            return True
    return False
