...
RULE_ID = "lighttpd.missing_strict_transport_security"
...
@rule(
    ...
    severity="medium",
    description="No Strict-Transport-Security header is configured via setenv.add-response-header.",
    recommendation="Add Strict-Transport-Security to setenv.add-response-header.",
    category="local",
...
def find_missing_strict_transport_security(
...
def _make_finding(config_ast: LighttpdConfigAst) -> Finding:
    return finding_from_rule(
        find_missing_strict_transport_security,
        location=default_location(config_ast),
    )


__all__ = ["find_missing_strict_transport_security"]
