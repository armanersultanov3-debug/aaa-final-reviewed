...
_DEPENDENCY_MANIFEST_BODY_PATTERNS = (
    ...
    r"""^gem\s+["']""",
    r"^GEM\s*$",
    r"<dependencies\b[\s\S]*?<dependency\b",
    r"<dependency\b[\s\S]*?<groupId\b[\s\S]*?<artifactId\b[\s\S]*?<version\b",
    r"\bdependencies\s*\{[\s\S]*?\b(?:api|implementation|compileOnly|runtimeOnly|testImplementation)\s*(?:\(|['\"])",
    r"<packages\b[\s\S]*?<package\b[\s\S]*?(?:\bid=|\bversion=)",
    r"<Package(?:Reference|Version)\b[\s\S]*?(?:\bInclude=|\bVersion=)",
    r"^module\s+\S+",
