...
_DEPENDENCY_MANIFEST_BODY_PATTERNS = (
    ...
    r"""^gem\s+["']""",
    r"^GEM\s*$",
    r"<dependencies\b[^>]*>[\s\S]*?<dependency\b[^>]*>[\s\S]*?<groupId\b[^>]*>[^<]+</groupId>[\s\S]*?<artifactId\b[^>]*>[^<]+</artifactId>[\s\S]*?<version\b[^>]*>[^<]+</version>[\s\S]*?</dependency>[\s\S]*?</dependencies>",
    r"\bdependencies\s*\{[\s\S]*?\b[A-Za-z_][\w.-]*\s*(?:\(\s*)?['\"][A-Za-z0-9_.-]+(?::[A-Za-z0-9_.-]+){1,2}['\"]",
    r"<packages\b[^>]*>[\s\S]*?<package\b(?=[^>]*\bid\s*=)(?=[^>]*\bversion\s*=)[^>]*/?>[\s\S]*?</packages>",
    r"<Package(?:Reference|Version)\b(?=[^>]*\bInclude\s*=)(?=[^>]*\bVersion\s*=)[^>]*/?>",
    r"^module\s+\S+",
...
SAFE_PATH_RULES: tuple[SafePathRule, ...] = (
    ...
    SafePathRule(
        ...
        standards=(
            ...
            _partial_asvs(
                "13.4.6",
                "Publicly exposed dependency manifest or lockfile version disclosure only.",
            ),
        ),
        order=698,
