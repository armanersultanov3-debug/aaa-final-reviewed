...
existing 332-rule inventory plus the rule-level work needed to cover the
...
- **Drift в счётчиках был выявлен и закрыт**: `docs/standards-roadmap.md`
  обновлён до 332 правил (Nginx 77, Apache 71, Lighttpd 47, IIS 51,
...
| STD-GAP-036 | Drift / sync счётчиков | direct-rule | P1 | done (2026-05-06) | 1 | ✓ Counters обновлены в `docs/standards-roadmap.md` (332 правил: Nginx 77, Apache 71, Lighttpd 47, IIS 51, External 73, Universal 13). Sync-check на блок «Total rules» в roadmap пока отложен — `tests/test_rule_coverage_doc.py` уже валидирует registry-vs-`rule-coverage.md`, добавление третьей точки потребует отдельного теста. |
