...
existing 396-rule inventory plus the rule-level work needed to cover the
...
- **Drift в счётчиках был выявлен и закрыт**: `docs/standards-roadmap.md`
  обновлён до 396 правил (Nginx 90, Apache 84, Lighttpd 49, IIS 52,
  External 107, Universal 14), чтобы совпадать с `docs/rule-coverage.md`.
...
| STD-GAP-028 | OWASP API Security Top 10 (2023) | covered | P3 | done (2026-05-14) | 10 | ✓ Большая часть out-of-scope для веб-сервера. После landing’а O-03 добавлена docs-only footnote/рационализация про secondary alignment with API7:2023 (SSRF) для `nginx.proxy_pass_user_controlled_destination` в `docs/rule-coverage.md`. |
...
| STD-GAP-036 | Drift / sync счётчиков | direct-rule | P1 | done (2026-05-06) | 1 | ✓ Counters обновлены в `docs/standards-roadmap.md` (396 правил: Nginx 90, Apache 84, Lighttpd 49, IIS 52, External 107, Universal 14). Sync-check реализован в `tests/test_rule_coverage_doc.py` (`test_repeated_document_counters_match_registry`): repeated counters в `README.md`, `docs/architecture.md`, `docs/standards-roadmap.md`, `docs/benchmarks-covering.md` и `docs/rule-coverage.md` валидируются против registry. |
