...
existing 293-rule inventory plus the rule-level work needed to cover the
...
- **Drift в счётчиках был выявлен и закрыт**: `docs/standards-roadmap.md`
  обновлён до 293 правил (Nginx 73, Apache 69, Lighttpd 20, IIS 47,
...
| STD-GAP-036 | Drift / sync счётчиков | direct-rule | P1 | done (2026-05-05) | 1 | ✓ Counters обновлены в `docs/standards-roadmap.md:40-49` (293 правила: Nginx 73, Apache 69, Lighttpd 20, IIS 47, External 73, Universal 11). Sync-check на блок «Total rules» в roadmap пока отложен — `tests/test_rule_coverage_doc.py` уже валидирует registry-vs-`rule-coverage.md`, добавление третьей точки потребует отдельного теста. |
