...
existing 421-rule inventory plus the rule-level work needed to cover the
...
| --- | --- | --- | --- |
| SC-8 / SC-8(1) | Transmission confidentiality and integrity | All TLS-presence and HTTP→HTTPS rules from §5.1 above plus `universal.tls_required_for_authenticated_routes` for Apache/Nginx auth-requiring routes on non-TLS or mixed listeners. | `parser-depth`: extend auth-requiring route normalization to IIS and Lighttpd (deferred to PR-08b). |
...
- **Drift в счётчиках был выявлен и закрыт**: `docs/standards-roadmap.md`
  обновлён до 421 правил (Nginx 85, Apache 84, Lighttpd 49, IIS 52,
  External 137, Universal 14), чтобы совпадать с `docs/rule-coverage.md`.
...
- **ASVS глава V8 (Authentication / Session)** теперь имеет прямое покрытие
  для Apache/Nginx auth-requiring routes через
  `universal.tls_required_for_authenticated_routes`, но IIS/Lighttpd и
  остальная глава всё ещё покрыты лишь частично (cookies, forms-auth IIS).
  Глава **V11 Cryptography** по-прежнему фрагментарна через V12.
...
- `closed (not pursued)` — окончательно отклонено решением плана
  `docs/superpowers/plans/2026-05-14-open-items-followup.md`; возвращаемся
  только при появлении конкретного аудитор-кейса.
...
| STD-GAP-020 | NIST CSF 2.0 | covered | P2 | docs-only alignment (plan 2026-05-14) | follow-up after O-03/O-05 | На уровне правил CSF даёт один и тот же тег на длинных группах. Финальное решение: docs-only topic-grouped блок «NIST CSF 2.0 alignment» в `docs/rule-coverage.md` (по образцу STD-GAP-026 / 032), без `StandardReference` записей на правилах. |
...
| STD-GAP-028 | OWASP API Security Top 10 (2023) | covered | P3 | docs-only (footnote after O-03) | follow-up after O-03 | Большая часть out-of-scope для веб-сервера. После landing’а O-03 (proxy_pass SSRF detection) — одна footnote-строка «secondary alignment with API7:2023 (SSRF)» на соответствующее новое правило в `docs/rule-coverage.md`. |
...
| STD-GAP-036 | Drift / sync счётчиков | direct-rule | P1 | done (2026-05-06) | 1 | ✓ Counters обновлены в `docs/standards-roadmap.md` (421 правило: Nginx 85, Apache 84, Lighttpd 49, IIS 52, External 137, Universal 14). Sync-check реализован в `tests/test_rule_coverage_doc.py` (`test_repeated_document_counters_match_registry`): repeated counters в `README.md`, `docs/architecture.md`, `docs/standards-roadmap.md`, `docs/benchmarks-covering.md` и `docs/rule-coverage.md` валидируются против registry. |
| STD-GAP-037 | ASVS V8 / V11 deepening | parser-depth | P3 | accepted via O-03/O-05 (plan 2026-05-14) | follow-up after O-03/O-05 | Та же глубина парсера, которая нужна для taint-model в O-03 (Nginx CRLF / SSRF) и source-span в O-05 (Lighttpd condition traceability), автоматически открывает V8 (защита данных в логах/заголовках) и V11 (redirect / method abuse). Реализуется как теги поверх новых правил в PR-3/PR-4. |
