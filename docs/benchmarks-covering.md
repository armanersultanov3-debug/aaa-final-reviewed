...
existing 273-rule inventory plus the rule-level work needed to cover the
...
- Use the existing gap labels: `covered`, `direct-rule`, `parser-depth`,
  `probe-depth`, `host-depth`, `out-of-scope`, `research`.
...
| §3.6 | TLS compression disabled | `apache.ssl_compression_enabled` | — | `direct-rule`: equivalent for Nginx (`gzip` over TLS is OK; CRIME applies to TLS-level compression, off by default in modern stacks) — gate behind explicit directive presence to avoid noise. |
...
| --- | --- | --- |
| §5.2 | Disable unneeded modules / services | All `external.server_status_exposed`, `apache.mod_status_public`, `lighttpd.mod_cgi_enabled`, `iis.webdav_module_enabled`, etc. (same set as CM-7). |
...
| DE.CM-09 | Computing hardware/software configurations monitored | not directly — `host-depth`. |
...
| 10.5 | Audit log retention / protection | none — `host-depth`. | — |
...
| --- | --- | --- |
| 8.20 | Networks security | All TLS rules + `apache.listen_requires_explicit_address`, `iis.binding_without_host_header`, `universal.listen_on_all_interfaces`. |
...
| APP.3.2.A3 | Sicheres Hochfahren | `host-depth`. | — |
...
| APP.3.2.A12 | Sichere Erhebung von Konfigurationsdaten | `host-depth`. | — |
| APP.3.2.A14 | Schutz von Web-Anwendungen und Web-Services über Reverse-Proxy | `nginx.proxy_missing_source_ip_headers` (partial). | `parser-depth`: full reverse-proxy upstream-TLS modelling (overlaps `STD-GAP-007`). |
...
| РСБ.3 | Сбор, запись и хранение информации о событиях | `nginx.log_format_missing_fields`, `apache.log_format_missing_fields`. | `direct-rule`: расширить требуемые поля под `request-id`, `x-forwarded-for`, TLS-version. |
| РСБ.7 | Защита информации о событиях | `host-depth` — права на лог-файлы. | — |
...
| АНЗ.2 | Контроль установки обновлений ПО | `host-depth`. | — |
| ЗИС.3 | Защита от внутренних и внешних угроз с использованием технологий межсетевого экранирования | `apache.listen_requires_explicit_address`, `iis.binding_without_host_header`, `universal.listen_on_all_interfaces`. | — |
...
- **Drift в счётчиках**: `docs/standards-roadmap.md:40-48` декларирует 270
  правил (Nginx 63, Apache 65, Lighttpd 15, IIS 44, External 72), но
  `docs/rule-coverage.md:32` уже показывает 273 (Nginx 64, Lighttpd 16,
  IIS 45). Roadmap-документ устарел.
- **`STD-GAP-012` "standards metadata в reports"** до сих пор `direct-rule, P2`
  — то есть группировка отчётов по стандартам ещё не доехала до пользователя,
  хотя инвентарь готов.
...
| --- | --- | --- | --- | --- | --- | --- |
| STD-GAP-016 | NIST SP 800-52 Rev. 2 | covered | P1 | done (2026-05-05) | 5 | ✓ Добавлен консолидированный блок «NIST SP 800-52 Rev. 2 mapping» в `docs/rule-coverage.md`. Покрыты §3.1 (TLS 1.2/1.3), §3.3.1 (cipher suites), §3.3.2 (server preference), §3.4 (certificates), §3.5 (renegotiation), §3.6 (compression), §4.2/§4.3 (OCSP stapling), §4.2.4 (HSTS), no-plaintext-fallback. Подход topic-grouped, не per-row column — совпадает с уже покрытыми TLS-правилами по CIS/CWE-327. Хелпер `nist_sp()` отложен до `STD-GAP-012` (code metadata только после стабилизации doc-mapping per `standards-roadmap.md:118-123`). |
...
| STD-GAP-021 | PCI DSS v4.0.1 | covered | P1 | done (2026-05-05) | 4 | ✓ Добавлен консолидированный блок «PCI DSS v4.0.1 mapping» в `docs/rule-coverage.md`. Покрыты Req. 2.2.1, 2.2.5, 2.2.6, 4.2.1, 6.2.4, 6.4.3, 8.3.1, 8.3.2, 8.3.5/8.3.6, 10.2.1, 10.2.2; Req. 10.5 / 12 явно отмечены как `host-depth` / `out-of-scope`. Подход topic-grouped, не per-row column. Хелпер `pci_dss_4()` отложен до `STD-GAP-012`. |
...
| STD-GAP-027 | OWASP Cheat Sheet Series | covered | P1 | done (2026-05-05) | 3 | ✓ Добавлен консолидированный блок «OWASP Cheat Sheet Series companions» в `docs/rule-coverage.md` (перед `## Standards mapping plan`). Подход topic-grouped, не per-row column: 14 cheat sheets (HTTP Security Response Headers, HSTS, TLS, CSP, CSRF, Session Management, Logging, Authentication, Credential Stuffing Prevention, Clickjacking Defense, Server Headers, Web Service Security, File Upload, Access Control, Error Handling) с aligned rule IDs. Cheat Sheets — living docs, поэтому отдельная колонка не вводится. |
...
| STD-GAP-036 | Drift / sync счётчиков | direct-rule | P1 | done (2026-05-05) | 1 | ✓ Counters обновлены в `docs/standards-roadmap.md:40-49` (270 → 273, Nginx 64, Lighttpd 16, IIS 45). Sync-check на блок «Total rules» в roadmap пока отложен — `tests/test_rule_coverage_doc.py` уже валидирует registry-vs-`rule-coverage.md`, добавление третьей точки потребует отдельного теста. |
