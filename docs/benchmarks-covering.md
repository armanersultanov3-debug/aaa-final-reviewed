...
| STD-GAP-037 | ASVS V8 / V11 deepening | parser-depth | P3 | deferred (PR-6 audit, 2026-05-14) | — | Аудит на PR-6 показал: текущие `asvs_5` references в коде используют V3 (web frontend), V12 (secure communications), V13 (configuration). V8 и V11 НЕ упоминаются ни одной существующей привязкой. План был основан на эвристическом сопоставлении "V8 = защита данных в логах/заголовках, V11 = redirect / method abuse", которое не совпадает с canonical ASVS 5.0.0 chapter numbering без authoritative source. Откладываем до отдельного research-PR, который сверит точные ID разделов 5.0.0 против OWASP ASVS 5.0.0 release notes и предложит маппинг без догадок. |
