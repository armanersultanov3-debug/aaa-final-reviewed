...
The current project inventory is 237 rules:
...
| STD-GAP-006 | Apache CIS | direct-rule | P2 | `FileETag`, timeout/keepalive values, request-limit thresholds, primary security-header checks, sensitive-location method restrictions, `AllowOverride None` baseline checks, sensitive-file deny lists, log-quality checks, and Apache TLS directive checks are now partially covered. Remaining direct-rule work focuses on full approved-method policy, environment-specific path policy, and deeper TLS redirect / HSTS / cipher-string validation. |
