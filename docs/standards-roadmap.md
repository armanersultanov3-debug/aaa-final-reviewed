...
The current project inventory is 248 rules:
...
| STD-GAP-009 | IIS / vendor docs | direct-rule | P2 | Host-header coverage, common authorization anonymous-access cases, Basic Authentication SSL coupling, and explicit unsafe request-filtering limits/deny-list toggles are now partially covered. Remaining IIS XML checks include application pools, full authorization defaults, cookie protection, credential storage, MachineKey/trust settings, handler permission semantics, native `Server` header behavior, and absence-complete request-filtering policy. |
