...
| STD-GAP-003 | Nginx CIS | direct-rule | P2 | Add remaining follow-up Nginx rules for benchmark items that current parser data can support, such as unknown-host rejection, HTTPS redirects, cipher-string validation, and remaining context-aware rate-limit placement checks. Timeout-value, unlimited-body-size, URI-buffer-floor, session-ticket, OCSP stapling completeness, and core connection/rate-limit validation checks are now present. |
