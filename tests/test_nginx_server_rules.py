...
def test_analyze_nginx_config_reports_first_tls_server_without_catch_all_reject(
    tmp_path: Path,
) -> None:
    config_path = tmp_path / "nginx.conf"
    config_path.write_text(
    ...
    matching = [
        ...
        if finding.rule_id == "nginx.default_tls_server_not_rejecting_unknown_hosts"
    ]
...
def test_analyze_nginx_config_accepts_first_tls_server_that_returns_444(
    tmp_path: Path,
) -> None:
    config_path = tmp_path / "nginx.conf"
    config_path.write_text(
...
def test_analyze_nginx_config_prefers_explicit_default_tls_server_over_first_named_server(
    tmp_path: Path,
) -> None:
    config_path = tmp_path / "nginx.conf"
    config_path.write_text(
...
def test_analyze_nginx_config_reports_missing_sensitive_config_file_restriction(
    tmp_path: Path,
) -> None:
    config_path = tmp_path / "nginx.conf"
    config_path.write_text(
...
def test_analyze_nginx_config_does_not_report_missing_sensitive_config_file_restriction_when_present(
    tmp_path: Path,
) -> None:
    config_path = tmp_path / "nginx.conf"
    config_path.write_text(
...
def test_analyze_nginx_config_does_not_treat_longer_extension_token_as_sensitive_config_coverage(
    ...
    config_path.write_text(
        ...
        "        deny all;\n"
        "    }\n"
        "    location ~* \\.(env|ini|log|orig|save|sql|tmp)$ {\n"
        "        deny all;\n"
        "    }\n"
        "}\n",
        encoding="utf-8",
    ...
    finding = next(
        finding
        for finding in result.findings
        if finding.rule_id == "nginx.sensitive_config_files_not_restricted"
    )
    assert ".conf" in finding.description
...
def test_analyze_nginx_config_reports_missing_sitewide_http_method_policy_for_root_proxy(
    tmp_path: Path,
) -> None:
    config_path = tmp_path / "nginx.conf"
    config_path.write_text(
...
def test_analyze_nginx_config_does_not_report_sitewide_http_method_policy_when_root_location_has_approved_limit_except(
    tmp_path: Path,
) -> None:
    config_path = tmp_path / "nginx.conf"
    config_path.write_text(
...
def test_analyze_nginx_config_does_not_report_sitewide_http_method_policy_when_root_location_uses_request_method_if_allowlist(
    tmp_path: Path,
) -> None:
    config_path = tmp_path / "nginx.conf"
    config_path.write_text(
...
def test_analyze_nginx_config_reports_missing_sitewide_http_method_policy_when_if_uses_unrelated_map_variable(
    tmp_path: Path,
) -> None:
    config_path = tmp_path / "nginx.conf"
    config_path.write_text(
...
def test_analyze_nginx_config_reports_missing_sitewide_http_method_policy_when_request_method_allowlist_differs_from_baseline(
    tmp_path: Path,
) -> None:
    config_path = tmp_path / "nginx.conf"
    config_path.write_text(
...
def test_analyze_nginx_config_reports_missing_sitewide_http_method_policy_when_limit_except_lacks_restrictive_body(
    tmp_path: Path,
) -> None:
    config_path = tmp_path / "nginx.conf"
    config_path.write_text(
...
def test_analyze_nginx_config_reports_missing_sitewide_http_method_policy_for_single_method_ban(
    tmp_path: Path,
) -> None:
    config_path = tmp_path / "nginx.conf"
    config_path.write_text(
