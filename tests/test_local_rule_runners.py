...
def test_run_universal_rules_collects_rule_execution_issue_and_continues(monkeypatch):
    ...
    monkeypatch.setattr(
        module.registry,
        "rules_for",
        lambda _category, server_type=None, include_opt_in_tags=(): [bad_entry, good_entry],
    )
...
def test_run_universal_rules_reraises_without_issue_collector(monkeypatch):
    ...
    monkeypatch.setattr(
        module.registry,
        "rules_for",
        lambda _category, server_type=None, include_opt_in_tags=(): [bad_entry],
    )
    ...
    def _fake_rules_for(
        ...
        *,
        ...
        if category == "local" and server_type == "nginx":
            return [bad_entry]
        return original_rules_for(
            category, server_type, include_opt_in_tags=include_opt_in_tags,
        )
